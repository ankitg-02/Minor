...
q AS (
    SELECT report_end_dt,
        ...
        CAST(
            date_add('day', -1, date_trunc('quarter', report_end_dt)) AS date
        ) AS prev_quarter_end_dt,
...
prev_open_non_etf AS (
    ...
    FROM standardmodel.aum_meview
        CROSS JOIN q
    WHERE fund = '125'
        AND q.fiscal_quarter > 1 -- use prev_quarter_end_dt as the creation/measurement date for previous-quarter open aum
        AND CAST(creation_date AS date) = q.prev_quarter_end_dt
        AND scheme NOT IN (
            SELECT distinct fm_scheme
            FROM bronze.indiabulls_dbo_mcr_schexclude_rt
        )
...
prev_open_intra_non_etf AS (
    SELECT gis.scheme AS scheme,
        SUM(a.netunits) AS openintraunits,
        SUM(a.aum) AS openintraaum
    FROM standardmodel.aum_dview a
        JOIN bronze.global_intra_scheme gis ON gis.fund = a.fund
        AND gis.folio = a.folio
        CROSS JOIN q
    WHERE a.fund = '125'
        AND q.fiscal_quarter > 1
...
curr_close_intra_non_etf AS (
    ...
    FROM standardmodel.aum_dview a
        ...
        AND gis.folio = a.folio
    WHERE a.fund = '125'
        AND CAST(a.batchclosedt AS date) = (
            SELECT report_end_dt
            FROM params
        )
    GROUP BY gis.scheme
        ...
        SUM(
            CASE
                WHEN trtype IN (
                    ...
                    'SWP',
                    'STP',
                    'RED' -- example outflow/trtype; adjust to your exact list if different
                ) THEN coalesce(redamt, 0)
                ELSE 0
            END
        ) AS total_outflow_amt
    FROM standardmodel.inout_flow_dview
    WHERE fund = '125' -- flows that happened within current quarter (inclusive)
        AND CAST(batchclosedt AS date) BETWEEN (
            SELECT curr_quarter_start_dt
            FROM q
        )
        AND (
            SELECT report_end_dt
...
non_etf_block AS (
    SELECT coalesce(p.scheme, c.scheme, b.scheme) AS scheme,
...
final_prep AS (
    ...
    FROM all_schemes a
        LEFT JOIN bronze.global_asset_master am ON am.fund_code = '125'
        AND am.scheme_code = a.scheme
    GROUP BY a.scheme,
        am.scheme_name,
        am.final_category
)
SELECT fp.scheme AS "Scheme Code",
    coalesce(fp.scheme_name, fp.scheme) AS "Scheme Name",
    coalesce(fp.final_category, 'Unknown') AS "Scheme Category (As per MCR)",
    (
        coalesce(fp.openaum, 0) - coalesce(fp.openintraaum, 0)
    ) / 1e7 AS "AUM as on last day of previous quarter (INR Cr)",
    fp.folio_count AS "Folio Count (Current Quarter End)",
    (coalesce(fp.puramt, 0) - 0) / 1e7 AS "Total Inflow (INR Cr)",
    -- adjust subtracting of liquidaum if needed
    (coalesce(fp.redamt, 0)) / 1e7 AS "Total Outflow (INR Cr)",
    (
        coalesce(fp.closeaum, 0) - coalesce(fp.closeintraaum, 0)
    ) / 1e7 AS "AUM as on last day of current quarter (INR Cr)"
FROM final_prep fp
ORDER BY fp.scheme;