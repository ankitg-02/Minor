...
dates AS (
    SELECT report_month,
        report_year,
        CASE
            WHEN report_month = '04' THEN NULL
            WHEN report_month = '07' THEN concat(report_year, '-03-31')
            WHEN report_month = '10' THEN concat(report_year, '-06-30')
            WHEN report_month = '01' THEN concat(
                cast(cast(report_year AS integer) - 1 AS varchar),
                '-09-30'
            )
            ELSE NULL
...
t1 AS (
    ...
    from (
            ...
            from (
                    ...
                    from (
                            select scheme,
                                sum(cast(aum as decimal(38, 3))) as openaum
                            from standardmodel.etf_aum_dview
                            where fund = '125'
                                and rundt = '2025-03-31'
                            group by 1
                        ) as a
                        full join (
                            select scheme,
                                count(distinct(scheme, plan, dpclid)) folio_count,
                                sum(cast(aum as decimal(38, 3))) as closeaum
                            from (
                                    select scheme,
                                        plan,
                                        dpclid,
                                        sum(cast(aum as decimal(38, 3))) as aum
                                    from standardmodel.etf_aum_dview
                                    where fund = '125'
                                        and rundt = '2025-06-30'
                                    group by 1,
                                        2,
                                        3
                                    having sum(netunits) > 0
                        ...
                        full join (
                            select scheme,
                                sum(
                                    case
                                        when purred = 'P' then apln_amount
                                        else 0
                                    end
                                ) as puramt,
                                sum(
                                    case
                                        when purred = 'R' then apln_amount
                                        else 0
                                    end
                                ) as redamt
...
prev_open_intra AS (
    ...
    FROM standardmodel.aum_dview a
        JOIN bronze.global_intra_scheme g ON g.folio = a.folio
        AND g.fund = a.fund
        JOIN dates_parsed d ON TRUE
    WHERE d.prev_quarter_end_date IS NOT NULL
...
SELECT y.scheme,
    ...
    CASE
        ...
        ELSE (
            coalesce(po.prev_openaum, 0) - coalesce(pi.prev_openintraaum, 0)
        ) / 1e7
