...
st.title("Yotube Comments Analysis Dashboard")
...
if st.button("Clean Text & Predict Sentiment"):
    if user_input.strip():
        ...
        if sentiment == "Positive":
            st.success(f"😊 The comment is **{sentiment}** (Good)")
        elif sentiment == "Negative":
            st.error(f"😞 The comment is **{sentiment}** (Bad)")
        else:
            st.info(f"😐 The comment is **{sentiment}** (Neutral)")
    else:
