...
def search_videos_by_keyword(keyword: str, max_results: int = 50, days_back: int = 60) -> List[str]:
    """
...
def fetch_comments(video_id: str, max_results: Optional[int] = None) -> List[Dict[str, Any]]:
    """
    Fetches comments from a specific YouTube video.

    Args:
        video_id (str): YouTube video ID.
        max_results (Optional[int]): Number of comments to retrieve. None fetches all comments.
    ...
    try:
        ...
        if "commentsDisabled" in str(e):
            print(f"⚠️  Comments disabled for video ID: {video_id}")
        else:
...
def save_comments(comments: List[Dict[str, Any]], keyword: str) -> None:
    """
...
def run_ingestion_pipeline(keyword="Youtube update", max_video_results=50, max_comment_results=100, days_back=60):
    """
...
if __name__ == "__main__":
    run_ingestion_pipeline(keyword="Youtube update")
