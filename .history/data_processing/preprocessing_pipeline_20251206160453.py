...
def load_raw_data(data: str) -> pd.DataFrame:
...
def save_processed_data(df: pd.DataFrame, keyword: str) -> str:
    """Saves processed comments to a date-based folder in data/processed."""
    # Create a timestamped folder for this run
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    run_folder = f"data/processed/{timestamp}"
    os.makedirs(run_folder, exist_ok=True)

    out_path = f"{run_folder}/cleaned_comments_{keyword}.csv"
    df.to_csv(out_path, index=False)
    print(f"💾 Cleaned data saved to: {out_path}")
    return run_folder


def run_preprocessing_pipeline(data: str, keyword: str = "YouTube") -> pd.DataFrame:
    """
...
if __name__ == "__main__":
    ...
    run_preprocessing_pipeline(latest_file, keyword="Youtube update")
